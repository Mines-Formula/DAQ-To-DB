...
def build_time_ref(file) -> int:
    ...
    while(len(Date)<6): # Edge case, if given 06/05/25 date value is "60525" convert to -> "060525"
        Date = "0" + Date
    ...
    if(int(str(Time)[0:2])>23): # Edge case, if given 3113036044 (HHMMSS.sss) we know that the data was read wrong, as big endian instead of little
...
def convert_to_unix(FILE_NAME: str, FILE_OUTPUT: str):
    ...
    time_ref:int = build_time_ref(FILE_NAME)
    header_written = False
    for chunk in pd.read_csv(
        FILE_NAME,
        dtype={"Timestamp": "int", "CANID": "string", "Sensor": "string", "Value": "string", "Unit": "string"},
        na_filter=False,
        chunksize=200000
    ):
