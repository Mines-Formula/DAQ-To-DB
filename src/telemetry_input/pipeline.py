...
from .config import InputConfig, InputFormat, ProtobufOutputMode
...
def decode_to_csv(
    ...
    if (
        config.input_format in {InputFormat.PROTOBUF_BINARY, InputFormat.PROTOBUF_DELIMITED}
        and config.protobuf_output_mode == ProtobufOutputMode.DECODED_TELEMETRY
    ):
        ...
        for record in records:
            ...
            writer.writerow(
                [
                    ...
                    "" if normalized.sensor is None else normalized.sensor,
                    normalized.value,
